...
V1 是单管理员使用的简易内部后台，已接入实际后端 `miao-travel-server` 的管理员登录、游戏用户查询、资料查看、云存档诊断/回滚、封禁/解封和脱敏错误日志接口。不开发管理员管理、复杂角色配置、通知中心、占位指标仪表盘、告警或工单系统。界面统一称“用户”，后端契约和路由继续沿用稳定的 `playerId` 与 `/admin/v1/players`，避免破坏现有接口。
