...
当前是单管理员使用的简易内部后台，已接入实际后端 `miao-travel-server` 的管理员登录、游戏用户查询、资料查看、云存档诊断/回滚、封禁/解封、脱敏错误日志和轻量公告管理接口。不开发管理员管理、复杂角色配置、通知中心、占位指标仪表盘、告警或工单系统。界面统一称“用户”，后端契约和路由继续沿用稳定的 `playerId` 与 `/admin/v1/players`，避免破坏现有接口。

公告管理按普通业务页面实现：列表分页、详情编辑、新增、更新、删除，富文本使用内置轻量工具栏；图片通过后端通用 `/admin/v1/files/upload` 上传，公告只保存返回的文件引用。COS 密钥不进入本项目。
...
若用户管理页为空，先确认使用的是 `npm run dev:cloud`。普通 `npm run dev` 默认连接本机 `127.0.0.1:3000`，本地内存数据库不会包含微信 CloudBase 的游戏用户。管理员登录账号属于独立的 `admin_users`，不会出现在游戏用户列表；游戏用户需要至少成功进入一次小游戏并完成平台登录，才会写入 `players`。
